...
import{	 getObjectsByPrototype,	 getTime,	 RANGED_ATTACK,	 HEAL,	 ATTACK,	 getDirection,	 getDistance,	 getTerrainAt,	 TERRAIN_WALL,	 TERRAIN_SWAMP}from '/game';
...
function checkForAssaults()
{
	 ...
	 if((enemiesInRange.length < 10 && getTime() > 1000) && enemyCreeps.length > 4)
	 {
			 ...
			 if(averageDistance > 8 && enemiesInRange.length == 0)
			 {
					 moveAllToRallypoint();
			 }
	 }
...
function checkForStaticDefence(creep)
{
	 ...
	 if((enemiesInRange.length > 10 && getTime() > 35) /* creeps hovering near their own base */ )
	 {
